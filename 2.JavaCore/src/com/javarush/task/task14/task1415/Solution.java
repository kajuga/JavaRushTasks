...
/*
Простите, но как то очень грустно смотреть на кучу if(). Лучше уж так:
1. Вводим общий метод для всех наследников в интерфейсе:
static interface Apartment { void clean(); }
2. В каждом наследнике в общий метод помещаем специфическое исполнение,
например: static class Apt1Room implements Apartment { ... public void clean() { clean1Room();
}
}
static class Apt2Room ... ... public void clean() { clean2Rooms();
}
} и т. п.
3. Вызываем один метод для всех классов: public static void cleanAllApartments(List apartments) {
for (Apartment ap : apartments) { ap.clean();
}
}
 */